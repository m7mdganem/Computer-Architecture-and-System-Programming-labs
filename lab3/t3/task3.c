...
short VIS = 0; // this short equals 0 iff we are in big endian and 1 otherwise
...
unsigned short convert(char firstElement, char secondElement){
    if(VIS==1){
        return (secondElement<<8 | firstElement); //convert to little endian
    }else{
        return (firstElement<<8 | secondElement); //convert to big endian
    }
}
...
virus* readVirus(FILE* input){
    ...
    vir->SigSize =convert(arr[0],arr[1]); //convert to little\big endian
